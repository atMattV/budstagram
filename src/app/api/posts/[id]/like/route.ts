...
export async function POST(
  ...
  let shouldSet = false
  if (!deviceId) { deviceId = crypto.randomUUID(); shouldSet = true }
  ...
  try {
    ...
    if (existing) {
      ...
      const updated = await prisma.post.findUnique({ where: { id: params.id }, select: { likes: true } })
    ...
    if (shouldSet) {
      res.cookies.set(COOKIE, deviceId, { httpOnly: true, sameSite: 'lax', secure: true, path: '/', maxAge })
    }
    return res
  } catch (err: any) {
